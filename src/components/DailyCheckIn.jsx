...
import { Form, Input, Select, DatePicker, TimePicker, Upload, Button, Card, Tabs, Table, Calendar, Statistic, Row, Col, message, Modal, Descriptions, Badge, Space, Tooltip, Timeline, Tag } from 'antd'
import { UploadOutlined, EditOutlined, HistoryOutlined, UserSwitchOutlined, ClockCircleOutlined } from '@ant-design/icons'
...
const CheckInForm = ({ currentUser, onSubmit }) => {
  ...
  useEffect(() => {
    ...
    const userList = Object.entries(allUsers).map(([account, user]) => ({
      ...user,
      account,
    ...
    const defaultOperator = {
      ...
      account: Object.keys(allUsers).find(key => allUsers[key].id === currentUser.id) || currentUser.id
    }
  ...
  // 处理作业人员变更
  const handleOperatorChange = (newOperatorId) => {
    const newOperator = users.find(u => u.id === newOperatorId)
    if (!newOperator || !originalOperator) return
    
    // 如果变更为非原始人员，记录变更
    if (newOperator.id !== originalOperator.id) {
      const changeRecord = {
        type: 'change',
        timestamp: new Date().toISOString(),
      ...
      message.info(`作业人员已变更为：${newOperator.name}，变更记录已保存`)
    } else {
      // 改回原始人员，移除后续变更记录
  ...
  const handleSubmit = (values) => {
    const selectedOperator = users.find(u => u.id === values.operator)
    ...
    const submitData = {
      ...
      operatorHistory: operatorHistory.filter(h => h.type === 'change'), // 只保存变更记录
      createdBy: {
        id: currentUser.id,
        name: currentUser.name
      },
    ...
    <div>
      <Form form={form} layout="vertical" onFinish={handleSubmit}>
        ...
        <Form.Item name="time" label="时间" rules={[{ required: true }]}>
          <TimePicker style={{ width: '100%' }} showSecond />
        </Form.Item>
        ...
        <Form.Item name="details" label="作业详情" rules={[{ required: true }]}>
          <Input.TextArea rows={4} placeholder="请详细描述作业内容" />
        </Form.Item>
...
const DailyCheckIn = () => {
  ...
  useEffect(() => {
    ...
    // 从localStorage获取打卡记录
    const records = checkInService.getAll()
        ...
        <TabPane tab="统计分析" key="statistics">
          <Card title="作业统计">
            <Row gutter={16}>
              ...
              <Col span={12}>
                <Statistic title="参与人员" value={personnel.length} />
              </Col>
            </Row>
