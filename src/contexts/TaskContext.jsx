...
export const TaskProvider = ({ children }) => {
  ...
  const toggleTaskStatus = (id) => {
    setTasks(prevTasks => {
      console.log('切换任务状态前:', prevTasks)
      const updatedTasks = prevTasks.map(task => 
        task.id === id ? { ...task, status: task.status === '待办' ? '已完成' : '待办' } : task
      )
      console.log('切换任务状态后:', updatedTasks)
      return updatedTasks
    })
  }
  ...
  const getPendingTasks = useCallback(() => {
    ...
    console.log('获取待办任务:', currentTasks.filter(task => task.status === '待办'))
    return currentTasks.filter(task => task.status === '待办')
  }, [getCurrentOrchardTasks])
